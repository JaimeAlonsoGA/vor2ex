...
export async function searchProducts(term: string): Promise<{
    products: Product[];
    niche: Niche;
}> {
    if (!term.trim()) {
    ...
    const [amazonApiData, scraperData, alibabaApiData] = await Promise.all([
        collectAmazonCatalogData(term, connection.endpoint, connection.marketplace),
        collectAmazonSearchData(term, connection.domain),
        collectAlibabaSearchData(term),
    ]);
    ...
    const niche = getNiche(term, amazonApiData, alibabaApiData, uniqueProducts);

    // Upsert nicho en background (no bloquea el SSR)
    upsertNiche(niche).catch(console.error);
